...
def run_walkforward_analysis(strategy, data: pd.DataFrame) -> Dict[str, Any]:
    ...
    if num_windows < 1:
        raise ValueError(
            f"Insufficient data: need at least {window_size} bars, have {total_bars}"
        )
    ...
    for i in range(num_windows):
        ...
        try:
            close_col = (
                test_df["close"] if "close" in test_df.columns else test_df.iloc[:, 0]
            )
        ...
        print(
            f"✅ Window {i + 1}: Train={train_sharpe:.3f}, Test={test_sharpe:.3f}, Hold={hold_sharpe:.3f}"
        )
...
def optimize_window(
    strategy_name: str, data: pd.DataFrame, param_grid: Dict
) -> Tuple[Dict, float]:
    """
