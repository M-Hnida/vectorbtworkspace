...
def create_visualizations(
    results: Dict[str, Any], strategy_name: str = "Trading Strategy"
) -> Dict[str, go.Figure]:
    """
    ...
    if portfolios:
        ...
        for name, pf in portfolios.items():
            figures[f"portfolio_{name}"] = _plot_single_portfolio(
                pf, name, strategy_name
            )

        # Plot comparison if multiple portfolios exist
        if len(portfolios) > 1:
            figures["portfolio_comparison"] = _create_comparison_plot(
                portfolios, strategy_name
            )

    # 2. Monte Carlo Analysis
    if mc_results := results.get("monte_carlo"):
        # Check if it's path MC (has simulated_returns) or parameter MC (has simulations)
        if "simulated_returns" in mc_results:
            # Path MC - only show path results
            figures["monte_carlo_paths"] = plot_path_mc_results(mc_results)
        elif "simulations" in mc_results:
            # Parameter MC - show dashboard
            figures["monte_carlo_dashboard"] = _plot_monte_carlo_dashboard(mc_results)
        else:
            print("⚠️  Warning: Unknown MC results format")
    ...
    if wf_results := results.get("walkforward"):
        ...
        if windows:
            ...
            if has_multiple_assets:
                figures["walk_forward_analysis"] = _plot_multi_asset_walkforward(
                    wf_results
                )
            else:
                figures["walk_forward_analysis"] = _plot_single_asset_walkforward(
                    wf_results
                )

    # 4. Strategy Optimization Comparison (Default vs Optimized)
    if "default_backtest" in results and "full_backtest" in results:
        figures["optimization_impact"] = _create_optimization_comparison(
...
def _create_comparison_plot(
    portfolios: Dict[str, Any], strategy_name: str
) -> go.Figure:
    ...
    ordered = sorted(
        portfolios.items(), key=lambda kv: (not kv[0].endswith("_default"), kv[0])
    )
...
def _plot_monte_carlo_dashboard(mc_results: Dict[str, Any]) -> go.Figure:
    ...
    fig.update_layout(
        title="Monte Carlo Analysis Dashboard", template="plotly_dark", height=900
    )
    return fig
...
def plot_path_mc_results(mc_results: Dict[str, Any]) -> go.Figure:
    ...
    fig.add_vline(
        ...
        line_dash="dash",
        line_color="red",
        annotation_text=f"Orig: {stats['original_return']:.2f}%",
        row=1,
        col=1,
    )
    ...
    fig.add_vline(
        ...
        line_dash="dash",
        line_color="red",
        annotation_text=f"Orig: {stats['original_sharpe']:.3f}",
        row=1,
        col=2,
    )
    ...
    fig.add_trace(
        go.Histogram(
            x=mc_results.get("simulated_max_dds", []),
            name="MC Max DD",
            nbinsx=50,
            marker_color="salmon",
            showlegend=False,
    ...
    if "original_max_dd" in stats:
        fig.add_vline(
            ...
            line_dash="dash",
            line_color="red",
            annotation_text=f"Orig: {stats['original_max_dd']:.2f}%",
            row=2,
            col=2,
        )
...
def _create_optimization_comparison(
    results: Dict[str, Any], strategy_name: str
) -> go.Figure:
...
def _plot_single_asset_walkforward(wf_results: Dict[str, Any]) -> go.Figure:
    ...
    fig.add_trace(
        go.Scatter(
            x=window_nums, y=train_sharpes, mode="lines+markers", name="Train Sharpe"
        ),
        row=1,
        col=1,
    )
    fig.add_trace(
        go.Scatter(
            x=window_nums, y=test_sharpes, mode="lines+markers", name="Test Sharpe"
        ),
        row=1,
        col=1,
    )
    fig.add_trace(
        go.Scatter(
            x=train_sharpes, y=test_sharpes, mode="markers", name="Sharpe Correlation"
        ),
        row=1,
        col=2,
    )
    ...
    fig.update_layout(
        title_text="Walk-Forward Analysis (Single Asset)", template="plotly_dark"
    )
    return fig
...
def _plot_multi_asset_walkforward(wf_results: Dict[str, Any]) -> go.Figure:
    ...
    for i, asset in enumerate(asset_names):
        ...
        train_returns = [
            w.get("asset_results", {}).get(asset, {}).get("train_return", 0)
            for w in windows
        ]
        test_returns = [
            w.get("asset_results", {}).get(asset, {}).get("test_return", 0)
            for w in windows
        ]
    ...
    asset_performance = [
        (
            asset,
            final_window.get("asset_results", {}).get(asset, {}).get("test_return", 0),
        )
    ...
    fig.add_trace(
        go.Bar(
            x=list(assets_sorted), y=list(returns_sorted), name="Final Test Returns"
        ),
        row=2,
        col=2,
    )
    fig.update_layout(
        title_text="Multi-Asset Walk-Forward Analysis",
        template="plotly_dark",
        showlegend=True,
    )
    return fig
...
def _add_parameter_sensitivity(fig: go.Figure, simulations: list) -> None:
    ...
    varying = [
        ...
        if len(set(v for v in param_data[n] if v is not None)) > 1
    ]
    if len(varying) == 2:
...
def _add_heatmap(
    fig: go.Figure, varying_params: list, param_data: dict, returns: pd.Series
):
    ...
    try:
        piv_pivot = (
            df.groupby(["yb", "xb"])["ret"]
            .mean()
            .reset_index()
            .pivot(index="yb", columns="xb", values="ret")
        )
        fig.add_trace(
            go.Heatmap(
                z=piv_pivot.values,
                ...
                colorscale="RdYlGn",
                colorbar=dict(title="Mean Return", x=1.1),
                name="Heatmap",
            ),
            row=1,
            col=2,
...
def _extract_parameter_data(simulations: list) -> Optional[dict]:
    if not simulations:
        return None
    first_sim = next(
        (s for s in simulations if isinstance(s, dict) and "parameters" in s), None
    )
    if not first_sim:
        return None
...
def _add_simulation_paths(
    fig: go.Figure, simulations: list, mc_results: Dict[str, Any]
) -> None:
...
def _add_mc_comparison(fig: go.Figure, statistics: dict) -> None:
    ...
    fig.add_trace(
        go.Bar(x=categories, y=values, marker_color=colors, name="Perf Comparison"),
        row=2,
        col=2,
    )
...
def plot_comprehensive_analysis(
    portfolios: Dict[str, Any], strategy_name: str, mc_results=None, wf_results=None
) -> None:
