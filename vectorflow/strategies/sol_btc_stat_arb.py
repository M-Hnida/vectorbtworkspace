...
def calculate_spread_indicators(
    sol_data: pd.DataFrame, btc_data: pd.DataFrame, params: Dict
) -> pd.DataFrame:
    ...
    aligned_data = pd.DataFrame(
        {"sol_close": sol_data["close"], "btc_close": btc_data["close"]}
    ).dropna()
    ...
    aligned_data["ratio_mean"] = (
        aligned_data["log_ratio"].rolling(lookback_period).mean()
    )
    aligned_data["ratio_std"] = aligned_data["log_ratio"].rolling(lookback_period).std()

    # Z-score of the spread
    aligned_data["zscore"] = (
        aligned_data["log_ratio"] - aligned_data["ratio_mean"]
    ) / aligned_data["ratio_std"]

    # Rolling correlation
    aligned_data["correlation"] = (
        aligned_data["sol_log"].rolling(lookback_period).corr(aligned_data["btc_log"])
    )

    # Volatility measures
    aligned_data["sol_volatility"] = (
        aligned_data["sol_log"].rolling(lookback_period).std()
    )
    aligned_data["btc_volatility"] = (
        aligned_data["btc_log"].rolling(lookback_period).std()
    )

    # Bollinger Bands for the ratio
    aligned_data["ratio_upper"] = aligned_data["ratio_mean"] + (
        2 * aligned_data["ratio_std"]
    )
    aligned_data["ratio_lower"] = aligned_data["ratio_mean"] - (
        2 * aligned_data["ratio_std"]
    )
...
def generate_stat_arb_signals(data: pd.DataFrame, params: Dict) -> tuple:
    ...
    for i in range(len(data)):
        ...
        # Exit conditions
            ...
            if (
                entry_time
                and (current_time - entry_time).total_seconds() / 3600
                < min_holding_period
            ):
                continue
...
def create_portfolio(data: pd.DataFrame, params: Dict = None) -> "vbt.Portfolio":
    ...
    try:
        from strategies.pairs_data_loader import (
            create_synthetic_pairs_data,
            validate_pairs_data,
        )
    except ImportError:
    ...
    if not all(col in data.columns for col in required_cols):
        ...
        if "close" in data.columns:
            print(
                "📊 Creating synthetic BTC data correlated with SOL for demonstration"
            )
            combined_data = create_synthetic_pairs_data(data, correlation=0.75)
        else:
            raise ValueError(
                "Data must contain either 'close' column or both 'sol_close' and 'btc_close' columns"
            )
    else:
        combined_data = data
    ...
    long_entries, long_exits, short_entries, short_exits = generate_stat_arb_signals(
        stat_data, params
    )
...
def calculate_pair_correlation(
    sol_data: pd.DataFrame, btc_data: pd.DataFrame, window: int = 30
) -> pd.Series:
...
def calculate_spread_metrics(data: pd.DataFrame) -> Dict:
    ...
    return {
        ...
        "zscore_reversions": len(
            zscore[(zscore.abs() > 2) & (zscore.shift(1).abs() < 1)]
        ),
