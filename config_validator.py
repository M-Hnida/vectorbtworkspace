...
def print_validation_results(strategy_name: str, validation: Dict[str, List[str]]) -> bool:
    """
...
def validate_and_fix_config(strategy_name: str, config: Dict) -> Dict:
    ...
    if validation['errors']:
        ...
        if expected_params and 'optimization_grid' in config:
            ...
            fixed_grid = {
                k: v for k, v in original_grid.items() 
                if k in expected_params
            }
            
            if fixed_grid != original_grid:
                removed = set(original_grid.keys()) - set(fixed_grid.keys())
                print(f"🔧 Auto-fix: Removed unused optimization params: {sorted(removed)}")
