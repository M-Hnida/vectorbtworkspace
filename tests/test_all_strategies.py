...
def load_test_data() -> Dict[str, pd.DataFrame]:
    ...
    try:
        ...
        data["BTC"] = btc_df.iloc[
            -(TEST_YEARS * 365 * 24) :
        ].copy()  # Approximate hourly bars
    ...
    try:
        ...
        # Get last N years of data
        data["EURUSD"] = eurusd_df.iloc[
            -(TEST_YEARS * 365 * 24) :
        ].copy()  # Approximate hourly bars
        print(f"✅ Loaded EURUSD: {len(data['EURUSD'])} bars from {EURUSD_DATA_PATH}")
    except Exception as e:
        raise FileNotFoundError(
            f"Failed to load EURUSD data from {EURUSD_DATA_PATH}: {e}"
        )
...
def print_summary_statistics(summary: Dict):
    ...
    for i, result in enumerate(ranked_return[:3], 1):
        print(
            f"  {i}. {result['strategy']:20} on {result['symbol']:8} → {result['total_return']:>8.2f}%"
        )
    ...
    for i, result in enumerate(ranked_sharpe[:3], 1):
        print(
            f"  {i}. {result['strategy']:20} on {result['symbol']:8} → {result['sharpe_ratio']:>7.3f}"
        )
    ...
    for i, result in enumerate(ranked_dd[:3], 1):
        print(
            f"  {i}. {result['strategy']:20} on {result['symbol']:8} → {result['max_drawdown']:>8.2f}%"
        )
...
def run_all_tests() -> Dict[str, Dict[str, Dict]]:
    ...
    for strategy_name in strategies:
        ...
        for symbol, df in data.items():
            current_test += 1
            print(
                f"[{current_test}/{total_tests}] Testing {strategy_name} on {symbol}...",
                end=" ",
            )
            ...
            try:
                ...
                print(
                    f"✅ Return: {result['total_return']:.2f}%, Sharpe: {result['sharpe_ratio']:.3f}"
                )
            except Exception as e:
